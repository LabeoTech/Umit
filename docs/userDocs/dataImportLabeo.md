...
* *BinningTemp*:\
  ...
  > Note: A note on spatial and temporal binning:\
  The maximum amount of spatial and temporal binning to be applied to an imaging data without significant loss of information has to take into account several factors including the optics of the imaging system, the signal source (e.g. calcium indicator vs intrinsic signals). For a discussion on how to optimize the recording spatial and temporal resolutions, please click [here_LinkNotFunctionalYet](/ht_optimize_binning.md).
