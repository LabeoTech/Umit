classdef EventsManager < handle
    ...
    methods
        ...
        function set.trigPolarity(obj, trigPolarity)
            % Set function for the trigger polarity.
            % There are two types:
            %   - positive: trigger characterized by raising and falling
            %   edges
            %   - negative: trigger characterized by falling and raising
            %   edges.
            
            assert(ismember(lower(trigPolarity), {'positive','negative'}),'Invalid trigger polarity! It must be either "positive" or "negative".')
            obj.trigPolarity = lower(trigPolarity);
        end
        ...
        function getTriggers(obj, varargin)
            ...
            %   FilterFreq (scalar positive): Cut-off frequency of Low-pass
            %       filter(Hz).
            %
            p = inputParser;
            addRequired(p,'obj');
            ...
            for ii = 1:length(obj.trigChanName)
                ...
                if strcmpi(obj.trigPolarity,'negative')
                    % Flip the signal if the triggers are negative:
                    sigMin = min(signal);sigMax = max(signal);
                    signal = (1 - (signal-sigMin)./(sigMax-sigMin)).*(sigMax-sigMin) + sigMin;
                end
                if FilterFreq
                    % Apply low-pass filter to the data at selected cut-off
                    % frequency:
                    signal = single(filtfilt(lpass.sosMatrix, lpass.ScaleValues, double(signal)')');
                end
                [tmstmp,chanState] = obj.detectTrig(signal);
            ...
            if b_verbose
                ...
                fprintf(['Total number of triggers: %d\nTotal number of conditions: %d\n' ...
                    'Average trial length (' strMapHigh(obj.trigPolarity) ' state): %0.3f s\nAverage inter-trial length (' strMapLow(obj.trigPolarity) ' state): %0.3f s\n'...
