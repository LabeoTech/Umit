classdef PipelineManager < handle
    ...
    methods
        ...
        function varargout = setOpts(obj,varargin)
            ...
            if isfield(obj.pipe(idxFunc), 'opts_vals')
                ...
                [out,state]= buildInputDlg(obj.pipe(idxFunc).name,fieldnames(obj.pipe(idxFunc).opts),currVals,defVals,listVals,typeVals);
            end
            if isempty(out)
                state = false;
                if nargout
        ...
        function varargout = showPipeSummary(obj)
            ...
            for i = 1:length(obj.pipe)
                ...
                if ~isempty(obj.pipe(i).inputSource)
                    str = [str, sprintf('Input Source : "%s"\n', obj.pipe(i).inputSource)];
                end
                str = [str, sprintf('--------------------\n')];
            end
            if nargout == 0
                    ...
                    if obj.b_skipSteps
                        ...
                        if isempty(thisSeq)
                            ...
                            dummyTask = struct('name',['Skipped sequence # ' num2str(obj.current_seq)],...
                                'inputSource',selFile,'saveFileName',selFile);                                                                                    
                                                       
                            %%%% TO CHANGE THIS:
                            obj.updatePipeLog(dummyTask,datetime('now'),true,txt);
        ...
        function fH = drawPipe(obj)
            ...
            if isempty(obj.pipe);return;end
            ...
            function [btnArr, sourceBtn] = createButtons
                ...
                for ii = 1:length(pp)                  
                    ...
                    btnArr(ii).Tooltip = genToolTipTxt(pp(ii));
                end
            ...
            function callSetOpts(src,~,obj,panel)
                ...
                if state
                    ...
                    src.Tooltip = genToolTipTxt(obj.pipe(ppIndx));
                ...
                for ii = 1:length(sequenceList)
                    ...
                    for jj = 1:length(this_seq)
                        ...
                        if ~strcmpi(this_seq(jj).inputSource,'_CURRENT_DATA_') && ~this_seq(jj).inputStepIndx
                            ...
                            if ~strcmpi(this_seq(jj).inputSource,'_FOLDER_')
            ...
            function tipTxt = genToolTipTxt(step)
            ...
            function chooseSaveFileName(src,~,obj)
                ...
                % Update tooltips:
                btn.Tooltip = genToolTipTxt(obj.pipe(idxFcn));
    ...
    methods (Access = {?DataViewer})
        ...
        function loadDataFromDummyProtocol(obj,data,filename,dataInfo)
            % LOADDATAFROMDUMMYPROTOCOL methods imports the imaging data from 
            ...
            % Load DataHistory
            obj.loadDataHistory(obj.ProtocolObj.SaveDir);
    ...
    methods (Access = private)
        ...
        function step = setSaveFilename(obj,step,b_genTmpFile)
            ...
            nameList = [{obj.pipe(~idxFcn).saveFileName}, getFileList(obj.SaveFolderList{1})'];
        ...
        function updatePipeLog(obj,info,b_completed,errMsg)
            ...
            thisLog = table({info.name},{info.inputSource},{strjoin(info.saveFileName,', ')}, b_completed, info.runDatetime,{MessageShort},{MessageLong},...
                'VariableNames', {'TaskName','InputSource','SavedFiles', 'Completed', 'RunDateTime', 'Messages_short','Messages'});
        ...
        function [newSeq,skippedSteps, selFile] = skipSteps(obj,thisSeq)
            ...
            function [outSeq,skipNames] = compareDataHistory(obj,seqIn, dhIn)
                ...
                for jj = 1:length(seqIn)
                    ...
                    thisHistory(jj).inputSource = seqIn(jj).inputSource;
        ...
        function loadInputFile(obj,step)
            ...
            disp(['Loading input file : ' step.inputSource])
            % Load the input file:            
        ...
        function saveDataToFile(obj,step,b_failed)
            ...
            indx = find(arrayfun(@(x) isequaln(step,x),obj.pipe));
            % Get the sequence up until the last step:
            idx_seq = [obj.pipe.seq] == step.seq;
            this_seq = obj.pipe(find(idx_seq,1,'first'):indx);
            %
            if b_failed                
                % Look back in pipeline for steps with "data" or "stats data"
                % as output and save the current data using the task's info:
                this_seq = this_seq(1:length(this_seq)-1);
                %
                ...
                % Get file name:
                saveFileName = obj.setSaveFilename(this_seq(ii),false);
            else                
                % Get file name:
                saveFileName = this_seq(end).saveFileName;
            end
            % Save current data to file:
            saveData(fullfile(obj.current_saveFolder,saveFileName),...
                obj.current_data);
