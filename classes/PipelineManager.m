classdef PipelineManager < handle
    ...
    methods
        ...
        function state = setParameters(obj, stepName)
            %SETOPTS Edit parameters of a step in the pipeline.
            %
            %   state = obj.setOpts()
            %   state = obj.setOpts(stepName)
        ...
        function executePipeline(obj)
            ...
            %
            %   SKIP CONTROL:
            %       - If obj.b_skipSteps == true  (default):
            ...
            %           executed, ignoring any existing files that could otherwise
            %           satisfy steps or branches.
            %
            %   GLOBAL LOG:
            ...
            % -------------------------------------------------------------
            % 0c) Reset current-run global summary log
            % -------------------------------------------------------------
            ...
            for f = 1:numel(obj.SaveFolderList)
                ...
                % ---------------------------------------------------------
                % 5) Apply history-based matching only if enabled
                % ---------------------------------------------------------
                if obj.b_skipSteps
                ...
                % ---------------------------------------------------------
                % 7) Summarize current folder execution into globalPipeLog
                % ---------------------------------------------------------
                ...
                % ---------------------------------------------------------
                % 8) Post-execution cleanup (folder-level)
                % ---------------------------------------------------------
                obj.executionTrace = [];
        ...
        function loadPipe(obj,varargin)
            ...
            % Restore selected settings if present
        ...
        function generateScript(obj, filename)
            ...
            for ii = 1:length(obj.pipe)
                ...
                if endsWith(obj.pipe(ii).inputSource,'.dat') || endsWith(obj.pipe(ii).inputSource,'.pdat')
        ...
        function varargout = drawPipe(obj)
            ...
            uimenu('Parent',cm,'Label','Undo saving','Callback',{@chooseSaveFileName,obj}); % Menu to undo saving.
            uimenu('Parent',cm,'Label','Delete step','Callback',{@deleteStep,obj});% Menu to delete the selected step.
            % Set container:
            Container.handle = fH;
            ...
            function deleteStep(src,~,obj)
                % This function calls the method "rmTask" and redrawns the
                % pipeline.
                btn = gco;
                idxFcn = btn.UserData;
                % Remove task from pipeline:
                obj.rmTask(obj.pipe(idxFcn).name,'fromSequence',obj.pipe(idxFcn).seq);
                % Delete figure;
    ...
    methods (Access = private)
        ...
        function saveDataHistory(obj, SaveFolder)
            ...
            for ii = nEntries:-1:1
                ...
                hasFile = isfield(dataHistory(ii), 'file') && ...
                    ~isempty(dataHistory(ii).file);
                ...
                file_i = char(string(dataHistory(ii).file));
            ...
            if removedCount > 0
                dataHistory = dataHistory(keepMask);
    ...
    methods (Access = private)
        ...
        function matchHistoryAndSetPreload(obj, folder)
            ...
            for iLeaf = 1:numel(leafIDs)
                ...
                % nextNodeID is the first unmatched DAG node after it.
                % ---------------------------------------------------------
                ...
                nextNodeID    = candidateNodeIDs(bestLen + 1);

                connIdx = find( ...
                    [obj.connections.sourceNodeID] == preloadNodeID & ...
                    [obj.connections.targetNodeID] == nextNodeID, ...
                    1, 'first');
    ...
    methods (Access = private)
        ...
        % Private methods currently not reached from any non-private
        % entry point based on static call-graph analysis.
        % -------------------------------------------------------------
        ...
        function handle = createDataHandle(dataOrFile, metaData)
