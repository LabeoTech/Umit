...
expAcq = '(Acq|RestingState)_\d*'; % For Acquisition ID
...
PathAcq = fullfile(obj.MainDir, namesSubj, [namesAcq{:}]);
tmpAcq = cellfun(@(a,b) cellfun(@(x,y) Acquisition(x,[],y), a,b), namesAcq, {PathAcq}, 'UniformOutput', false);
...
cellfun(@(a,b) arrayfun(@(x,y) setfield(x.Array.ObjList(1), 'RawFolder', y{:}),a,b),...
    tmpAcq, FolderLabeo, 'UniformOutput', false);
cellfun(@(a,b) arrayfun(@(x,y) setfield(x.Array.ObjList(1), 'RawFiles', y{:}),a,b),...
