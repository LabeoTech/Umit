function genVAmask(data, metaData, SaveFolder, varargin)
...
default_opts = struct('fileName', 'VisualCtxAreas.mat', 'b_UseMask', false, 'MaskFile','ImagingReferenceFrame.mat');
opts_values = struct('fileName', {{'VisualCtxAreas.mat'}},'b_UseMask', [true,false], 'MaskFile',{{'ImagingReferenceFrame.mat'}});%#ok  % This is here only as a reference for PIPELINEMANAGER.m.
default_object = ''; % This line is here just for Pipeline management to be able to detect this input.
%%% Arguments parsing and validation %%%
p = inputParser;
addRequired(p,'data',@(x) isnumeric(x) & ismatrix(x)); % Validate if the input is a 2-D numerical matrix:
...
if ~endsWith(opts.fileName, '.mat')
    opts.fileName = [opts.fileName, '.mat'];
end
...
filename = fullfile(SaveFolder, opts.fileName);
